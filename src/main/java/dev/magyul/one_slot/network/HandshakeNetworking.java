        ...
        ServerLoginConnectionEvents.QUERY_START.register((handler, server, sender, synchronizer) -> {
            for (PacketInfo<? extends IHandshakeMessage> packetInfo : HANDSHAKE_PACKETS) {
                try {
                    ...
                    OneSlot.LOGGER.error("{} Handshake packet processing error", packetInfo.type.getId().toString(), e);
                }
            }
        });
    }
            ...
            try {
                if (buf.readableBytes() > 0) {
                    ...
                    if (ackPacketClass == null) {
                        OneSlot.LOGGER.error("{} Is the handshake response packet registered?", packetId.toString());
                        return;
                    }
                    IHandshakeMessage.IResponsePacket packet = ackPacketClass.getDeclaredConstructor().newInstance();
                    packet.read(buf);
                    packet.handle(handler.getConnectionInfo(), responseSender);
                }
            } catch (InstantiationException | IllegalAccessException | InvocationTargetException |
                     NoSuchMethodException e) {
                OneSlot.LOGGER.error("Handshake response packet processing error in {}", type.getId().toString(), e);
            }
        });
